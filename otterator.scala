package moorhouse{ //https://littleplasticsquares.com/2016/03/12/otterator/
    ...
    val two= new `ettoretto`(1).tter.andThen { x => 1 }(0)
  	//jgs' otter, thanks
  	val ootttteerr = List(`torettorettoretto`(`o`(18), `toretto`(two), `o`(2), `ttoretto`(1))
  			,`torettorettoretto`(`o`(8), `rettoretto`(two), `etto`(two), `tto`(two), `to`(1), `tto`(1), `etto`(1), `rettoretto`(1), `o`(3), `toretto`(1), `o`(1), `ttoretto`(1))
  			,`torettorettoretto`(`o`(7), `ttoretto`(1), `o`(6), `torettoretto`(two), `ettorettoretto`(1), `o`(2),`toretto`(1),  `ttoretto`(1))
  			,`torettorettoretto`(`o`(6), `toretto`(1), `o`(4), `torettoretto`(two), `o`(3), `ttorettoretto`(two), `oretto`(1), `rettoretto`(two), `etto`(1), `rettoretto`(1))
  			,`torettorettoretto`(`o`(6), `toretto`(1), `o`(7), `rettoretto`(1), `etto`(1), `orettoretto`(1), `retto`(1), `rettorettoretto`(1), `ttoretto`(1), `o`(5), `rettoretto`(1), `etto`(1), `rettoretto`(1))
  			,`torettorettoretto`(`o`(7), `ettorettoretto`(1), `o`(5), `ttoretto`(1), `o`(2), `ttoretto`(1), `oretto`(2), `rettoretto`(1), `etto`(3), `rettoretto`(1), `rettorettoretto`(1), `toretto`(1), `o`(2),`ettoretto`(1), `ettorettoretto`(1),`o`(3), `orettorettoretto`(1))
  			,`torettorettoretto`(`o`(8), `to`(1), `rettoretto`(two), `o`(2), `to`(1), `o`(2), `ttoretto`(two), `retto`(2), `o`(7), `ettoretto`(1), `to`(1), `etto`(1), `rettoretto`(1), `ttoretto`(1), `o`(1), `rettorettoretto`(1), `ttoretto`(1),`toretto`(1))
  			,`torettorettoretto`(`o`(10), `toretto`(1), `o`(2), `rettoretto`(1), `to`(1), `o`(2), `oretto`(1), `o`(9), `rettoretto`(1), `etto`(1), `rettoretto`(1), `orettoretto`(1), `ettoretto`(1), `o`(1), `ttoretto`(1))
  			,`torettorettoretto`(`o`(11), `to`(1), `rettoretto`(1), `o`(13), `toretto`(two), `o`(2), `ettoretto`(two),`ettorettoretto`(1), `etto`(1),`to`(1))
  			,`torettorettoretto`(`o`(13), `to`(two), `rettoretto`(two), `rettorettoretto`(1), `o`(8),  `etto`(1),`to`(1), `o`(4), `ttoretto`(1))
  			,`torettorettoretto`(`o`(16), `ettoretto`(2), `tto`(2), `etto`(2), `ettoretto`(1), `etto`(6),  `ettoretto`(1)))
  	...
  	implicit class otterator(val `::`: String) {
  		...
  		}.++((ootttteerr map (o => o.tter))./:("")(_+toChar.toChar+_) )
  	}
	}
}